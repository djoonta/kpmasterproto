// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.31.0
// 	protoc        v4.25.1
// source: master.proto

package kpmasterproto

import (
	installment "github.com/djoonta/kpmasterproto/installment"
	occupation "github.com/djoonta/kpmasterproto/occupation"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_master_proto protoreflect.FileDescriptor

var file_master_proto_rawDesc = []byte{
	,
}

var file_master_proto_goTypes = []interface{}{
	(*occupation.OccupationFindIDRequest)(nil),     // 0: kpmasterproto.OccupationFindIDRequest
	(*occupation.OccupationFindAllRequest)(nil),    // 1: kpmasterproto.OccupationFindAllRequest
	(*occupation.OccupationCreateRequest)(nil),     // 2: kpmasterproto.OccupationCreateRequest
	(*occupation.OccupationDeleteRequest)(nil),     // 3: kpmasterproto.OccupationDeleteRequest
	(*occupation.OccupationUpdateRequest)(nil),     // 4: kpmasterproto.OccupationUpdateRequest
	(*installment.InstallmentFindAllRequest)(nil),  // 5: kpmasterproto.InstallmentFindAllRequest
	(*installment.InstallmentFindIDRequest)(nil),   // 6: kpmasterproto.InstallmentFindIDRequest
	(*installment.InstallmentCreateRequest)(nil),   // 7: kpmasterproto.InstallmentCreateRequest
	(*installment.InstallmentDeleteRequest)(nil),   // 8: kpmasterproto.InstallmentDeleteRequest
	(*installment.InstallmentUpdateRequest)(nil),   // 9: kpmasterproto.InstallmentUpdateRequest
	(*occupation.OccupationFindIDResponse)(nil),    // 10: kpmasterproto.OccupationFindIDResponse
	(*occupation.OccupationFindAllResponse)(nil),   // 11: kpmasterproto.OccupationFindAllResponse
	(*occupation.OccupationCreateResponse)(nil),    // 12: kpmasterproto.OccupationCreateResponse
	(*occupation.OccupationDeleteResponse)(nil),    // 13: kpmasterproto.OccupationDeleteResponse
	(*occupation.OccupationUpdateResponse)(nil),    // 14: kpmasterproto.OccupationUpdateResponse
	(*installment.InstallmentFindAllResponse)(nil), // 15: kpmasterproto.InstallmentFindAllResponse
	(*installment.InstallmentFindIDResponse)(nil),  // 16: kpmasterproto.InstallmentFindIDResponse
	(*installment.InstallmentCreateResponse)(nil),  // 17: kpmasterproto.InstallmentCreateResponse
	(*installment.InstallmentDeleteResponse)(nil),  // 18: kpmasterproto.InstallmentDeleteResponse
	(*installment.InstallmentUpdateResponse)(nil),  // 19: kpmasterproto.InstallmentUpdateResponse
}
var file_master_proto_depIdxs = []int32{
	0,  // 0: kpmasterproto.OccupationService.FindID:input_type -> kpmasterproto.OccupationFindIDRequest
	1,  // 1: kpmasterproto.OccupationService.FindAll:input_type -> kpmasterproto.OccupationFindAllRequest
	2,  // 2: kpmasterproto.OccupationService.Create:input_type -> kpmasterproto.OccupationCreateRequest
	3,  // 3: kpmasterproto.OccupationService.Delete:input_type -> kpmasterproto.OccupationDeleteRequest
	4,  // 4: kpmasterproto.OccupationService.Update:input_type -> kpmasterproto.OccupationUpdateRequest
	5,  // 5: kpmasterproto.InstallmentService.FindAll:input_type -> kpmasterproto.InstallmentFindAllRequest
	6,  // 6: kpmasterproto.InstallmentService.FindID:input_type -> kpmasterproto.InstallmentFindIDRequest
	7,  // 7: kpmasterproto.InstallmentService.Create:input_type -> kpmasterproto.InstallmentCreateRequest
	8,  // 8: kpmasterproto.InstallmentService.Delete:input_type -> kpmasterproto.InstallmentDeleteRequest
	9,  // 9: kpmasterproto.InstallmentService.Update:input_type -> kpmasterproto.InstallmentUpdateRequest
	10, // 10: kpmasterproto.OccupationService.FindID:output_type -> kpmasterproto.OccupationFindIDResponse
	11, // 11: kpmasterproto.OccupationService.FindAll:output_type -> kpmasterproto.OccupationFindAllResponse
	12, // 12: kpmasterproto.OccupationService.Create:output_type -> kpmasterproto.OccupationCreateResponse
	13, // 13: kpmasterproto.OccupationService.Delete:output_type -> kpmasterproto.OccupationDeleteResponse
	14, // 14: kpmasterproto.OccupationService.Update:output_type -> kpmasterproto.OccupationUpdateResponse
	15, // 15: kpmasterproto.InstallmentService.FindAll:output_type -> kpmasterproto.InstallmentFindAllResponse
	16, // 16: kpmasterproto.InstallmentService.FindID:output_type -> kpmasterproto.InstallmentFindIDResponse
	17, // 17: kpmasterproto.InstallmentService.Create:output_type -> kpmasterproto.InstallmentCreateResponse
	18, // 18: kpmasterproto.InstallmentService.Delete:output_type -> kpmasterproto.InstallmentDeleteResponse
	19, // 19: kpmasterproto.InstallmentService.Update:output_type -> kpmasterproto.InstallmentUpdateResponse
	10, // [10:20] is the sub-list for method output_type
	0,  // [0:10] is the sub-list for method input_type
	0,  // [0:0] is the sub-list for extension type_name
	0,  // [0:0] is the sub-list for extension extendee
	0,  // [0:0] is the sub-list for field type_name
}

func init() { file_master_proto_init() }
func file_master_proto_init() {
	if File_master_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_master_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   2,
		},
		GoTypes:           file_master_proto_goTypes,
		DependencyIndexes: file_master_proto_depIdxs,
	}.Build()
	File_master_proto = out.File
	file_master_proto_rawDesc = nil
	file_master_proto_goTypes = nil
	file_master_proto_depIdxs = nil
}
